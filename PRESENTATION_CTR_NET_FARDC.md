...
**Message clé**
Trois profils structurent l’accès et les responsabilités opérationnelles.
...
- `CONTROLEUR`: saisie ciblée des contrôles via interface simplifiée.
...
- `CONTROLEUR` est orienté vers la saisie de contrôle (`modules/controles/ajouter.php`).
...
- Vérification de doublon sur matricule: un militaire déjà contrôlé est bloqué.
...
- Cas `CONTROLEUR`: accès direct saisie contrôle.
...
1. **Pourquoi trois profils et pas un profil unique ?**
...
2. **Le CONTROLEUR peut-il accéder au dashboard ?**
   Non, il est orienté vers la saisie de contrôle pour rester focalisé sur son rôle.
