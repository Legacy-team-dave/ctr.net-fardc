...
if ($garnisonLabelFromClient !== '') {
    ...
    $sourceLabel = 'Equipe : ' . $baseSourceLabel;
}
...
if (!empty($pendingControles)) {
 ...
 $summary = $hasRemoteConflicts
    ? 'Synchronisation transmise : ' . $remotePendingConflicts . ' conflit(s) en attente d\'arbitrage.'
...
sync_progress_event('progress', [
    ...
    'step'       => $hasRemoteConflicts
        ...
        : ($isRemoteAlreadySynced
            ? 'Données déjà présentes sur le serveur.'
...
sync_json_response(true, $hasRemoteConflicts
    ? 'Synchronisation transmise avec conflits en attente.'
    : ($isRemoteAlreadySynced ? 'Données déjà présentes sur le serveur central.' : 'Synchronisation terminée avec succès.'), 200, null, [
    'summary'           => $summary,
    'sync_state'        => $syncState,
