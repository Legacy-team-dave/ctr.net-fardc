...
function verifier_acces($roles_requis, $message_erreur = null) {
    ...
    if (!in_array($user_profil, $roles_requis)) {
        $_SESSION['flash_message'] = [
            'type' => 'danger',
            'text' => $message_erreur ?? 'Accès refusé. Vous n\'avez pas les droits nécessaires.'
        ];
...
<head>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('click', function(e) {
            const link = e.target.closest('a[data-required-role]');
            if (!link) return;

            let requiredRoles = link.getAttribute('data-required-role');
            const rolesAllowed = requiredRoles.split(',').map(r => r.trim());
