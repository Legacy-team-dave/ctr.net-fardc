...
<head>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function runSync() {
            ...
            syncBtn.innerHTML =
                '<i class="fas fa-spinner fa-spin"></i><span class="d-none d-lg-inline ml-1">Synchronisation...</span>';

            try {
                const response = await fetch(
                    '<?= htmlspecialchars($appBasePath) ?>/api/synchronisation.php?action=push', {
                        method: 'POST',
        ...
        if (syncBtn) {
            syncBtn.addEventListener('click', function(e) {
                ...
                if (window.confirm(
                        'Confirmez-vous la synchronisation des données locales vers le serveur central ?'
                    )) {
                    runSync();
                }
            });
