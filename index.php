                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-modern">
                                            ...
                                            <tbody>
                                                ...
                                                ?>
                                                32
                                                32<i class="fas <?= $icone ?>"
                                                    style="color: <?= $couleur ?>; margin-right: 8px;"></i>
                                                <?= htmlspecialchars($mention) ?>32
                                                32<?= number_format($m['total'] ?? 0, 0, ',', ' ') ?>32
                                                32<?= $pourcentage ?>%32
                                                32
                                                <?php endforeach; ?>
                                            </tbody>
                            ...
                            <div class="table-responsive mt-4">
                                <table class="table table-modern">
                                    ...
                                    <tbody>
                                        ...
                                        32
                                        32<strong><?= htmlspecialchars($stat['garnison']) ?></strong>32
                                        32<?= number_format($stat['total'] ?? 0, 0, ',', ' ') ?>32
                                        32
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-modern">
                                            ...
                                            <tbody>
                                                ...
                                                32<i class="fas <?= $icone ?>"
                                                    style="color: <?= $couleur ?>; margin-right: 8px;"></i>
                                                <?= htmlspecialchars($categorie_libelle) ?>32
                                                32<?= number_format($stat['total'] ?? 0, 0, ',', ' ') ?>32
                                                32<?= $pourcentage ?>%32
                                                32
                                                <?php endforeach; ?>
                        ...
                        <div class="card-body p-0">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    32<?= h($row['noms']) ?>32
                                    32<?= h($row['grade']) ?>32
                                    32<?= h($row['unite'] ?? 'N/A') ?>32
                                    32<?= h($row['province'] ?? 'N/A') ?>32
                        ...
                        <div class="card-body p-0">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                    32
                                    32<?= date('d/m/Y', strtotime($log['date_controle'])) ?>32
                                    32<?= h($log['noms']) ?><br><small><?= h($log['grade'] ?? '') ?>
                                        <?= !empty($log['unite']) ? '- ' . h($log['unite']) : '' ?></small>32
                                    32<span class="badge bg-<?= $mentionClass ?>"><i
                                            class="fas <?= $mentionIcon ?> mr-1"></i>
                                        <?= h($mention ?: 'Non spécifié') ?></span>32
...
<!-- qrcode.min.js retiré car non utilisé -->
...
function createOptimizedInfoControl() {
    ...
    info.clearInfo = function() {
        this._div.innerHTML = '<span>Survolez une province pour voir les effectifs</span>';
    };
...
function getZoneFilterLabel() {
    if (nonVusZones.length === 0) return "LISTE DES MILITAIRES NON-VUS AU CONTROLE";
    if (nonVusZones.length === 1) return `LISTE DES MILITAIRES NON-VUS AU CONTROLE DE LA ${nonVusZones[0]}`;
    const firstPart = nonVusZones.slice(0, -1).join(', ');
    const last = nonVusZones[nonVusZones.length - 1];
    return `LISTE DES MILITAIRES NON-VUS AU CONTROLE DE LA ${firstPart} ET ${last}`;
}
...
async function exportNonVusPDF() {
    ...
    try {
        ...
        link.download = `non_vus_${getTimestamp()}.pdf`;
        link.click();
        Swal.close();
        Swal.fire({
            icon: 'success',
...
function exportNonVusExcel() {
    ...
    XLSX.writeFile(wb, `non_vus_${getTimestamp()}.xlsx`);
    Swal.fire({
        icon: 'success',
        title: 'Export réussi',
...
async function exportNonVusZIP() {
    ...
    try {
        ...
        link.download = `non_vus_${getTimestamp()}.zip`;
        link.click();
