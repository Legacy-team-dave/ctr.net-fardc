...
<!-- Bibliothèques pour les exports (SheetJS, jsPDF, JSZip) -->
    ...
    function createOptimizedInfoControl() {
        ...
        info.updateWithStats = function(provinceName, stats) {
            ...
            <div class="stats-mini">
                ...
                <div><i class="fas fa-user-slash
"></i> <span>Inactifs :</span> <strong>${stats.inactifs || 0}</strong></div>
    ...
    <?php if (!$is_admin_ig): ?>
        ...
        function exportNonVusCSV() {
            ...
            const rows = nonVusData.map((m, index) => [
                ...
                m.garnison,
                m.province,
                categorieTraductions[m.categorie] || m.categorie, // Traduction de la catégorie
                m.zdef
            ]);
            const csvContent = [
        ...
        async function generateNonVusPDFBlob() {
            if (!nonVusData || nonVusData.length === 0) return null;
            const headerLines = [
                'MINISTERE DE LA DEFENSE NATIONALE ET ANCIENS COMBATTANTS',
                'INSPECTORAT GENERAL DES FARDC',
                getZoneFilterLabel()
        ...
        // Export ZIP des non-vus (contient CSV, Excel, PDF)
