...
require_login();
check_profil(['ADMIN_IG']); // Adaptez le profil si nécessaire (ex: ADMIN)

$page_titre = 'Importer des utilisateurs';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['import_data'])) {
    ...
    if ($data && is_array($data)) {
        $resultat = importerUtilisateurs($data);
...
/**
 * Insère les données dans la table utilisateurs
 * Structure: id_utilisateur, login, mot_de_passe, nom_complet, email, avatar, reset_token, reset_expires, profil, actif, dernier_acces, preferences, created_at
 */
function importerUtilisateurs($data)
{
    global $pdo;
    $compteur = 0;
    $erreurs = [];
    ...
    // Champs obligatoires pour l'import (les autres sont optionnels ou auront des valeurs par défaut)
    $obligatoires = ['login', 'nom_complet', 'email', 'profil'];

    // Récupération des logins et emails existants
    try {
        $stmt = $pdo->query("SELECT login, email, nom_complet FROM utilisateurs");
        $utilisateursExistants = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $loginsExistants = array_column($utilisateursExistants, 'login');
        $emailsExistants = array_column($utilisateursExistants, 'email');
        $infosExistants = [];
        foreach ($utilisateursExistants as $u) {
            $infosExistants[$u['login']] = $u['nom_complet'] . ' (' . $u['email'] . ')';
        }
    ...
    foreach ($data as $index => $ligne) {
        $indexLigne = $index + 2;
        $login = trim($ligne['login'] ?? '');
        $email = trim($ligne['email'] ?? '');
        $nom_complet = trim($ligne['nom_complet'] ?? '');
        ...
        if (empty($login)) {
            $erreurs[] = "Ligne $indexLigne : login vide";
            $stats['champs_manquants']++;
            continue;
        }
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $erreurs[] = "Ligne $indexLigne : email invalide ($email)";
            $stats['champs_manquants']++;
            continue;
        }

        // Vérification doublons dans le fichier
        if (in_array($login, $loginsDansFichier)) {
        ...
        if (in_array($email, $emailsDansFichier)) {
            $erreurs[] = "Ligne $indexLigne : email '$email' en double dans le fichier";
            $stats['doublons_fichier']++;
            continue;
        }

        // Vérification doublons dans la base
        if (in_array($login, $loginsExistants)) {
            $doublons_details['base'][] = [
                'ligne' => $indexLigne,
                'login' => $login,
                'nom_complet' => $nom_complet,
                'email' => $email,
                'existant' => $infosExistants[$login] ?? 'Inconnu'
            ];
            $stats['doublons_base']++;
            continue;
        }
        ...
        if (in_array($email, $emailsExistants)) {
            $erreurs[] = "Ligne $indexLigne : email '$email' existe déjà en base";
            $stats['doublons_base']++;
            continue;
        }

        $loginsDansFichier[] = $login;
        $emailsDansFichier[] = $email;
        $lignesValides[] = $ligne;
        $stats['valides']++;
    }
    ...
    $sql = "INSERT INTO utilisateurs 
            (login, mot_de_passe, nom_complet, email, avatar, reset_token, reset_expires, profil, actif, dernier_acces, preferences, created_at) 
            VALUES 
            (:login, :mot_de_passe, :nom_complet, :email, :avatar, :reset_token, :reset_expires, :profil, :actif, :dernier_acces, :preferences, NOW())";
    $stmt = $pdo->prepare($sql);

    foreach ($lignesValides as $index => $ligne) {
        $indexLigne = array_search($ligne, $data) + 2;

        // Mot de passe par défaut = login (à changer à la première connexion)
        $mot_de_passe_default = password_hash($ligne['login'], PASSWORD_DEFAULT);

        $params = [
            ':login' => trim($ligne['login']),
            ':mot_de_passe' => !empty(trim($ligne['mot_de_passe'] ?? '')) ? password_hash(trim($ligne['mot_de_passe']), PASSWORD_DEFAULT) : $mot_de_passe_default,
            ':nom_complet' => trim($ligne['nom_complet']),
            ':email' => trim($ligne['email']),
            ':avatar' => !empty(trim($ligne['avatar'] ?? '')) ? trim($ligne['avatar']) : null,
            ':reset_token' => null,
            ':reset_expires' => null,
            ':profil' => trim($ligne['profil']),
            ':actif' => isset($ligne['actif']) ? (int)$ligne['actif'] : 1, // 1 par défaut
            ':dernier_acces' => null,
            ':preferences' => !empty(trim($ligne['preferences'] ?? '')) ? trim($ligne['preferences']) : null
        ];
        ...
        try {
            ...
            if ($e->errorInfo[1] == 1062) {
                // Doublon sur login ou email
                $erreur_info = strpos($e->getMessage(), 'login') !== false ? 'login' : 'email';
                $doublons_details['base'][] = [
                    'ligne' => $indexLigne,
                    'login' => $ligne['login'],
                    'nom_complet' => $ligne['nom_complet'],
                    'email' => $ligne['email'],
                    'existant' => "Contrainte base ($erreur_info)"
                ];
                $stats['doublons_base']++;
            } else {
    ...
    if ($compteur > 0) {
        log_action('IMPORT', 'utilisateurs', null, "$compteur utilisateurs importés");
    }

    $message = "$compteur utilisateur(s) importé(s) sur " . count($data) . " ligne(s).";
...
<div class="container-fluid">
    <div class="modern-card">
        <div class="card-header">
            <h3><i class="fas fa-upload"></i> Importer des utilisateurs</h3>
        ...
        <div class="card-body">
                    ...
                    <div class="doublons-section">
                        ...
                        <table class="doublons-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><?= $d['ligne'] ?></td>
                                    <td><strong><?= htmlspecialchars($d['login']) ?></strong></td>
            ...
            <form method="post" id="importForm">
                ...
                <div class="form-text">
                    <i class="fas fa-info-circle"></i>
                    <strong>Champs requis :</strong> Login, Nom_complet, Email, Profil<br>
                    <small><i class="fas fa-key"></i> Si mot_de_passe non fourni, le login sera utilisé comme mot de
                        passe par défaut</small>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function afficherApercu(headers, data) {
        ...
        const required = ['login', 'nom_complet', 'email', 'profil'];
    ...
    importBtn.addEventListener('click', function() {
        ...
        Swal.fire({
            title: 'Confirmation',
            text: `Importer ${currentData.length} utilisateur(s) ?`,
            icon: 'question',
