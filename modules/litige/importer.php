...
$page_titre = 'Importer des litiges';
...
/**
 * Insère les données dans la table litiges
 * Champs obligatoires : matricule, noms, nom_beneficiaire
 * Doublons détectés sur (matricule, noms)
 */
function importerLitiges($data)
{
    global $pdo;
    $compteur = 0;
    $erreurs = [];
    ...
    $obligatoires = ['matricule', 'noms', 'nom_beneficiaire'];
    // Les autres champs sont optionnels

    // Récupération des litiges existants pour détection des doublons (sur matricule + noms)
    try {
        $stmt = $pdo->query("SELECT matricule, noms, nom_beneficiaire FROM litiges");
        $litigesExistants = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $existantsKeyed = [];
        foreach ($litigesExistants as $l) {
            $key = $l['matricule'] . '|' . $l['noms'];
            $existantsKeyed[$key] = $l['nom_beneficiaire'];
        }
    ...
    foreach ($data as $index => $ligne) {
        ...
        $nom_beneficiaire = trim($ligne['nom_beneficiaire'] ?? '');
        ...
        if (empty($matricule) || empty($noms) || empty($nom_beneficiaire)) {
            $erreurs[] = "Ligne $indexLigne : matricule, noms ou nom_beneficiaire vide";
            $stats['champs_manquants']++;
            continue;
        }

        // Clé unique dans le fichier : matricule|noms
        $cleFichier = $matricule . '|' . $noms;
        if (in_array($cleFichier, $clesDansFichier)) {
        ...
        if (isset($existantsKeyed[$cleFichier])) {
            $doublons_details['base'][] = [
                'ligne' => $indexLigne,
                'matricule' => $matricule,
                'noms' => $noms,
                'nom_beneficiaire' => $nom_beneficiaire,
                'existant' => $existantsKeyed[$cleFichier]
            ];
            $stats['doublons_base']++;
            continue;
        }

        $clesDansFichier[] = $cleFichier;
        $lignesValides[] = $ligne;
        $stats['valides']++;
    }
    ...
    // Insertion des données (les champs optionnels sont mis à NULL si absents)
    $sql = "INSERT INTO litiges 
            (matricule, noms, type_controle, nom_beneficiaire, lien_parente, garnison, province, date_controle, observations) 
            VALUES 
            (:matricule, :noms, :type_controle, :nom_beneficiaire, :lien_parente, :garnison, :province, :date_controle, :observations)";
    ...
    foreach ($lignesValides as $index => $ligne) {
        ...
        $params = [
            ':matricule' => trim($ligne['matricule']),
            ':noms' => trim($ligne['noms']),
            ':type_controle' => isset($ligne['type_controle']) && trim($ligne['type_controle']) !== '' ? trim($ligne['type_controle']) : null,
            ':nom_beneficiaire' => trim($ligne['nom_beneficiaire']),
            ':lien_parente' => isset($ligne['lien_parente']) && trim($ligne['lien_parente']) !== '' ? trim($ligne['lien_parente']) : null,
            ':garnison' => isset($ligne['garnison']) && trim($ligne['garnison']) !== '' ? trim($ligne['garnison']) : null,
            ':province' => isset($ligne['province']) && trim($ligne['province']) !== '' ? trim($ligne['province']) : null,
            ':date_controle' => isset($ligne['date_controle']) && trim($ligne['date_controle']) !== '' ? trim($ligne['date_controle']) : null,
            ':observations' => isset($ligne['observations']) && trim($ligne['observations']) !== '' ? trim($ligne['observations']) : null
        ];
        ...
        try {
            ...
            // Gestion des doublons sur contrainte unique (si elle existe)
            if ($e->errorInfo[1] == 1062) {
                $doublons_details['base'][] = [
                    'ligne' => $indexLigne,
                    'matricule' => $ligne['matricule'],
                    'noms' => $ligne['noms'],
                    'nom_beneficiaire' => $ligne['nom_beneficiaire'],
...
<div class=" container-fluid">
    <div class="modern-card">
        <div class="card-header">
            <h3><i class="fas fa-upload"></i> Importer des litiges</h3>
        ...
        <div class="card-body">
            ...
            <form method="post" id="importForm">
                ...
                <div class="form-text">
                    <i class="fas fa-info-circle"></i>
                    <strong>Champs requis :</strong> Matricule, Noms, Nom_beneficiaire<br>
                    <strong>Champs optionnels :</strong> Type_controle, Lien_parente, Garnison, Province, Date_controle,
                    Observations<br>
                    <small class="text-muted">Le fichier doit être encodé en UTF-8 (avec ou sans BOM). Les doublons sont
                        détectés sur le couple (Matricule, Noms).</small>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function lireCSV(file) {
            ...
            reader.onload = function(e) {
                ...
                // Supprimer le BOM UTF-8 (caractère \uFEFF) s'il est présent
        ...
        function afficherApercu(headers, data) {
            ...
            const required = ['matricule', 'noms', 'nom_beneficiaire'];
        ...
        importBtn.addEventListener('click', function() {
            ...
            Swal.fire({
                title: 'Confirmation',
                text: `Importer ${currentData.length} litige(s) ?`,
                icon: 'question',
