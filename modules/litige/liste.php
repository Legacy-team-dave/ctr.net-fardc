...
// Calcul des provinces et garnisons distinctes
$provinces = [];
$garnisons = [];
...
foreach ($litiges as $l) {
    ...
    // Collecte des provinces et garnisons distinctes
    if (!empty($l['province'])) {
        $provinces[] = $l['province'];
    }
    if (!empty($l['garnison'])) {
        $garnisons[] = $l['garnison'];
    }
}
$total_litiges = count($litiges);
$nb_provinces = count(array_unique($provinces));
$nb_garnisons = count(array_unique($garnisons));

// Récupération des garnisons uniques pour le filtre
$garnisons_list = array_values(array_unique(array_filter(array_column($litiges, 'garnison'))));
sort($garnisons_list);
...
$export_fields = [
    ...
    'grade'            => ['label' => 'Grade', 'enabled' => true, 'required' => false],
...
<style>
    ...
    /* Icône recherche devant le champ */
    .search-icon {
        color: #2e7d32;
        font-size: 0.9rem;
        margin-right: 8px;
...
<div class="container-fluid py-3">
    ...
    <!-- PREMIER BLOC DE STATISTIQUES (Total, Garnisons, Provinces) -->
    <div class="stats-container mb-4">
        ...
        <div class="stat-card">
            <div class="stat-icon garnison"><i class="fas fa-map-pin"></i></div>
            <div class="stat-info">
                <h4><?= $nb_garnisons ?></h4>
                <p>Garnisons</p>
            </div>
        ...
        <div class="stat-card">
            <div class="stat-icon province"><i class="fas fa-map-marked-alt"></i></div>
            <div class="stat-info">
                <h4><?= $nb_provinces ?></h4>
                <p>Provinces</p>
            </div>
    ...
    <!-- DEUXIÈME BLOC DE STATISTIQUES (Zones de défense) -->
            ...
            <div class="card modern-card">
                ...
                <div class="card-body">
                    ...
                    <table id="table-litiges" class="table-militaires" style="width:100%">
                        <thead>
                            32<th><input type="checkbox" class="select-all-checkbox" id="select-all"></th>
                            <th>Matricule</th>
                            <th>Noms</th>
                            <th>Grade</th>
                            <th>Type contrôle</th>
    ...
    $(document).ready(function() {
        ...
        const table = $('#table-litiges').DataTable({
            ...
            initComplete: function() {
                ...
                // Ajout de l'icône devant le champ de recherche
                const searchInput = searchLabel.find('input');
                searchLabel.empty(); // Supprime tout contenu existant
                const searchIcon = $('<i class="fas fa-search search-icon"></i>');
                searchLabel.append(searchIcon, searchInput);

                // Ajout du bouton "Nouveau litige"
                filterDiv.append(`
        ...
        function extractRowData(row) {
            ...
            // Indices après ajout de la colonne Grade
