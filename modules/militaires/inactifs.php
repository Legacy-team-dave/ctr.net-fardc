...
$page_titre = 'Liste des militaires inactifs';
$breadcrumb = ['Militaires inactifs' => '#'];
...
$stats = [
    ...
    'retraites' => $pdo->prepare("SELECT COUNT(*) FROM militaires WHERE statut = ? AND categorie = 'RETRAITES'")->execute([$statut_cible]) ? $pdo->query("SELECT COUNT(*) FROM militaires WHERE statut = $statut_cible AND categorie = 'RETRAITES'")->fetchColumn() : 0,
    'integres' => $pdo->prepare("SELECT COUNT(*) FROM militaires WHERE statut = ? AND categorie = 'INTEGRES'")->execute([$statut_cible]) ? $pdo->query("SELECT COUNT(*) FROM militaires WHERE statut = $statut_cible AND categorie = 'INTEGRES'")->fetchColumn() : 0,
    'dcd_av_bio' => $pdo->prepare("SELECT COUNT(*) FROM militaires WHERE statut = ? AND categorie = 'DCD_AV_BIO'")->execute([$statut_cible]) ? $pdo->query("SELECT COUNT(*) FROM militaires WHERE statut = $statut_cible AND categorie = 'DCD_AV_BIO'")->fetchColumn() : 0,
    'dcd_ap_bio' => $pdo->prepare("SELECT COUNT(*) FROM militaires WHERE statut = ? AND categorie = 'DCD_AP_BIO'")->execute([$statut_cible]) ? $pdo->query("SELECT COUNT(*) FROM militaires WHERE statut = $statut_cible AND categorie = 'DCD_AP_BIO'")->fetchColumn() : 0
];
...
$top_retraites = $pdo->prepare("
    SELECT grade, COUNT(*) as total 
    FROM militaires 
    WHERE statut = ? AND categorie = 'RETRAITES' 
    GROUP BY grade 
    ORDER BY total DESC 
    LIMIT 5
");
$top_retraites->execute([$statut_cible]);
$top_retraites = $top_retraites->fetchAll();

$top_integres = $pdo->prepare("
    SELECT grade, COUNT(*) as total 
    FROM militaires 
    WHERE statut = ? AND categorie = 'INTEGRES' 
    GROUP BY grade 
    ORDER BY total DESC 
    LIMIT 5
");
$top_integres->execute([$statut_cible]);
$top_integres = $top_integres->fetchAll();

$top_dcd_av_bio = $pdo->prepare("
    SELECT grade, COUNT(*) as total 
    FROM militaires 
    WHERE statut = ? AND categorie = 'DCD_AV_BIO' 
    GROUP BY grade 
    ORDER BY total DESC 
    LIMIT 5
");
$top_dcd_av_bio->execute([$statut_cible]);
$top_dcd_av_bio = $top_dcd_av_bio->fetchAll();

$top_dcd_ap_bio = $pdo->prepare("
    SELECT grade, COUNT(*) as total 
    FROM militaires 
    WHERE statut = ? AND categorie = 'DCD_AP_BIO' 
    GROUP BY grade 
    ORDER BY total DESC 
    LIMIT 5
");
$top_dcd_ap_bio->execute([$statut_cible]);
$top_dcd_ap_bio = $top_dcd_ap_bio->fetchAll();
...
<div class="container-fluid py-3">
    ...
    <div class="stats-section">
        <div class="section-title">
            <i class="fas fa-chart-pie"></i> Statistiques Générales (Inactifs)
        </div>
        ...
        <div class="col-12">
            <div class="card modern-card">
                <div class="card-header">
                    <h3 class="mb-0"><i class="fas fa-users"></i> Militaires inactifs</h3>
...
$(document).ready(function() {
    ...
    $('#export-csv').on('click', async function() {
        try {
            ...
            link.download = `militaires_inactifs_${getTimestamp()}.csv`;
    ...
    $('#export-excel').on('click', async function() {
        try {
            ...
            const link = document.createElement('a');
            link.href = URL.createObjectURL(blob);
            link.download = `militaires_inactifs_${getTimestamp()}.xlsx`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            URL.revokeObjectURL(link.href);
...
<?php
audit_action('CONSULTATION', 'militaires', null, 'Consultation de la liste des militaires inactifs');
