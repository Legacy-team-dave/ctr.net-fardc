...
<head>
    ...
    * {
    ...
    .item {
        ...
        border-radius: 4px;
        padding: 2px 4px;
        /* légèrement réduit pour 3 colonnes */
        cursor: pointer;
        transition: all 0.2s;
        font-size: 0.7rem;
        /* un peu plus petit */
...
"></i> Garnisons <span id="garnisonTotal"><?= count($garnisons) ?></span></h4>
                            ...
                            <div class="grid" id="garnisonsContainer">
                                ...
                                <label class="item">
                                    ...
                                    <span class="item-name"><?= htmlspecialchars($g['garnison']) ?></span>
        ...
        fetch('ajax/get_categories.php', {
            ...
            .then(data => {
                ...
                if (data.length === 0) {
                    ...
                    data.forEach(cat => {
                        html +=
                            `<label class="item"><input type="checkbox" name="categories[]" value="${cat.categorie}" class="categorie-input"><span class="item-name">${traductions[cat.categorie] || cat.categorie}</span><span class="item-count">${cat.total}</span></label>`;
                    });
